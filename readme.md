...
- **Automated Trading**: Filled orders are executed on-chain according to the atomic swap protocol designed by Garden.
- **Gas Optimized**: COBI uses the latest gas projections from indexers to ensure that transactions are confirmed quickly and cheaply.
- **Delegator Rewards**: COBI can be run by anyone who has a filler role in the garden governance system. The delegates who voted for the filler in the garden staking system will receive a portion of the trading fees generated by COBI in exchange for increasing credibility as a filling in the orderbook AMM.
